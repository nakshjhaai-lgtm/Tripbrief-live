...
function bindAfterRender() {
  addEventListener('screen:rendered', () => {
    document.getElementById('clipboardBtn')?.addEventListener('click', async () => { try { toast('Saved offline. Reading quietly…'); await importClipboard(); await refreshState(); render(); toast('Clipboard saved. Pack refreshed quietly.'); } catch(e) { toast(e.message || 'Clipboard unavailable. Paste manually.'); } });
    document.getElementById('saveTextBtn')?.addEventListener('click', async () => { const text = document.getElementById('manualText').value; if (!text.trim()) return toast('Paste anything travel-related first.'); toast('Saved offline. Reading quietly…'); await saveText(text, 'manual', 'Manual paste'); await refreshState(); render(); toast('Text saved. Trip gently updated.'); });
    document.getElementById('fileInput')?.addEventListener('change', async e => { toast('Saved offline. Reading quietly…'); await saveFiles(e.target.files, 'upload').catch(err=>toast(err.message)); await refreshState(); render(); toast('Files saved. Pack refreshed quietly.'); });
    document.getElementById('cameraInput')?.addEventListener('change', async e => { toast('Photo saved offline. Reading quietly…'); await saveFiles(e.target.files, 'camera').catch(err=>toast(err.message)); await refreshState(); render(); });
